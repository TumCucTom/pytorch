...
cos: aten.cos.default -> PREFER_RECOMPUTE""",
    ...
    def test_region_activation_memory_budget_inside_invoke_subgraph(self):
        """A budget set inside an invoke_subgraph (nested_compile_region) body is
        applied to that body: the HOP body is partitioned with its own budget,
        independently of the enclosing graph."""
        ...
        def build(budget):
            ...
            def fn(x):
                # The reduction stays outside the region -> the enclosing graph
                # is unannotated, so only the HOP body carries a budget.
                return region(x).sum()
